...
typedef OBX_OPTIONS(unsigned int, OBXPropertyFlags) {
    ...
    OBXPropertyFlags_idCompanion = 16384,
...
typedef OBX_ENUM(unsigned int, OBXPutPaddingMode) {
...
/// Configure an offset and a limit for this query - all methods that support an offset/limit will return/process objects starting at this
/// offset and up to the given limit.
/// Example use case: get a slice of the whole result, e.g. for "result paging".
/// Call with offset/limit=0 to reset to the default behavior, i.e. starting from the first element without limit.
